    ...
    namespace filter {
        ...
        public:
            ...
            static double bhattacharyyaDistance(const UKF<Model>& a, const UKF<Model>& b) {
                ...
                const typename UKF<Model>::StateMat& s1 = a.getCovariance();
                const typename UKF<Model>::StateMat& s2 = b.getCovariance();
                typename UKF<Model>::StateMat s         = (s1 + s2) * 0.5;

                double d = ((0.125 * ud).transpose() * s.inverse() * ud)
                           + 0.5 * std::log(s.determinant() / std::sqrt(s1.determinant() * s2.determinant()));
                return d;
            }
        ...
        public:
            ...
            void measurementUpdate(
                const std::initializer_list<std::tuple<TMeasurement, Eigen::MatrixXd, TMeasurementArgs...>>&
                    measurements) {
