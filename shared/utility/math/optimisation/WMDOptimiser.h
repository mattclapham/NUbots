...
namespace math {
    namespace optimisation {
        ...
        public:
             ...
             */
            WMDEstimator(const Eigen::VectorXd& params)
                : startPoint(params)
                , bestEstimate(params){
             ...
             * @author Josiah Walker
             */
            Eigen::VectorXd updateEstimate(const Eigen::MatrixXd& samples,
                ...
                for (size_t j = 0; j < 50; ++j) {
                    Eigen::VectorXd update = Eigen::VectorXd::Zero(bestEstimate.size());
                    double div             = 0.0;

                    for (size_t i = 0; i < samples.rows(); ++i) {
                        const double scale =
                            sampleWeights[i] / (bestEstimate - samples.row(i).transpose().norm() + 0.01);
        ...
        public:
             ...
             * @author Josiah Walker
             */
            WMDSampler(const Eigen::VectorXd& params) : sigmaWeights(params) {}
            ...
            updateEstimate(const Eigen::MatrixXd& samples,
                           const Eigen::VectorXd& fitnesses,
                           const Eigen::VectorXd& currentEstimate) {
                ;  // unused
            }
             ...
             * @author Josiah walker
             */
            Eigen::MatrixXd getSamples(const Eigen::VectorXd& bestEstimate, const size_t& numSamples) {
