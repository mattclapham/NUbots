...
namespace localisation {
    ...
    Eigen::Matrix<double, FieldModel::size, 1> FieldModel::timeUpdate(const Eigen::Matrix<double, size, 1>& state,
                                                                      double /*deltaT*/) {
        return state;
    }

    Eigen::VectorXd FieldModel::predictedObservation(
        ...
        for (auto& type : measurements) {
            ...
            switch (std::get<0>(type)) {
                ...
                case GoalTeam::OWN:
                    ...
                    switch (std::get<1>(type)) {
                        case GoalSide::LEFT:
                            goalLocation.head<2>() = field.goalpost_own_l;
                            // ans += " left";
                            break;
                        case GoalSide::RIGHT:
                            goalLocation.head<2>() = field.goalpost_own_r;
                            // ans += " right";
                            break;
                ...
                case GoalTeam::OPPONENT:
                    ...
                    switch (std::get<1>(type)) {
                        case GoalSide::LEFT:
                            // ans += " left";
                            goalLocation.head<2>() = field.goalpost_opp_l;
                            break;
                        case GoalSide::RIGHT:
                            // ans += " right";
                            goalLocation.head<2>() = field.goalpost_opp_r;
                            break;
                        case GoalSide::UNKNOWN_SIDE: break;
                    }
                    break;
            ...
            if (lastGoalLocation.cwiseNotEqual(goalLocation).any()) {
            ...
            switch (std::get<2>(type).value) {
                ...
                case GoalMeasurementType::LEFT_NORMAL: {
                    prediction.middleRows(counter, 3) = goalNormals.col(0);
                } break;

                case GoalMeasurementType::RIGHT_NORMAL: {
                    prediction.middleRows(counter, 3) = goalNormals.col(1);
                } break;

                case GoalMeasurementType::TOP_NORMAL: {
                    prediction.middleRows(counter, 3) = goalNormals.col(2);
                } break;

                case GoalMeasurementType::BASE_NORMAL: {
                    prediction.middleRows(counter, 3) = goalNormals.col(3);
                } break;
                default: break;
            }
            counter += 3;
    ...
    Eigen::Matrix<double, FieldModel::size, 1> FieldModel::limitState(
        const Eigen::Matrix<double, size, 1>& state) const {
        return state;
    }
