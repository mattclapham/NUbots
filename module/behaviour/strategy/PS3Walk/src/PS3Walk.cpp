        ...
        PS3Walk::PS3Walk(std::unique_ptr<NUClear::Environment> environment)
            ...
            on<Every<20, Per<std::chrono::seconds>>>().then([this] {
                ...
                if (moving) {
                    ...
                    // Eigen::Vector2d s = { std::max(strafe[0], 0.0), strafe[1] };
                    Eigen::Vector2d s          = strafe;
                    Eigen::Vector2d strafeNorm = s / std::numeric_limits<short>::max();

                    double rotationalSpeedNorm = rotationalSpeed / std::numeric_limits<short>::max();
                    Transform2D transform{strafeNorm, rotationalSpeedNorm};
