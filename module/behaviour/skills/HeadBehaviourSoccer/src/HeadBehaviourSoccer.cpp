...
namespace behaviour {
    namespace skills {
               ...
               Sync<HeadBehaviourSoccer>>()
                .then("Head Behaviour Main Loop",
                      [this](const Sensors& sensors,
                          ...
                          bool updatePlan = !isGettingUp && ((lastBallPriority != ballPriority)
                          ...
                          if (!lost) {
                              ...
                              if (ballMaxPriority) {
                                  headToBodyRotation =
                                      Transform3D(ballFixationObjects[0].visObject.sensors->forwardKinematics.at(
                                                      ServoID::HEAD_PITCH))
                                          .rotation();
                                  orientation =
                                      Transform3D(ballFixationObjects[0].visObject.sensors->world).rotation().inverse();
                              }
                              else {
                                  headToBodyRotation =
                                      Transform3D(goalFixationObjects[0].visObject.sensors->forwardKinematics.at(
                                                      ServoID::HEAD_PITCH))
                                          .rotation();
                                  orientation =
                                      Transform3D(goalFixationObjects[0].visObject.sensors->world).rotation().inverse();
                              }
                          }
                          else {
                              headToBodyRotation =
                                  Transform3D(sensors.forwardKinematics.at(ServoID::HEAD_PITCH)).rotation();
        ...
        void HeadBehaviourSoccer::updateHeadPlan(const KinematicsModel& kinematicsModel,
            ...
            Eigen::Vector2d currentPos = {sensors.servo.at(ServoID::HEAD_YAW).presentPosition,
            ...
            for (uint i = 0; i < fixationObjects.size(); i++) {
                ...
                centroid += fixationObjects[i].visObject.screenAngular / fixationObjects.size();
            }
