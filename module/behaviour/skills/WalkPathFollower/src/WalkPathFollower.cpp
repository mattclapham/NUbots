...
namespace behaviour {
    namespace skills {
        ...
        WalkCommand WalkPathFollower::walkBetweenNear(const Transform2D& currentState, const Transform2D& targetState) {
            ...
            ///     Transform2D velocity = {cfg_.walk_to_near_speed * (targetState.xy() -
            ///     currentState.xy()).normalize(), rotationSpeed}; //TODO make 20 seconds the variable update_frequency
            ///     WalkCommand command(subsumptionId, velocity);
            ///     return command;
            // } else {
            //     Eigen::Vector2d strafe = { std::max(cfg_.walk_about_x_strafe, 0.0), -angleSign *
            //     cfg_.walk_about_y_strafe };
