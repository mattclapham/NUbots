...
namespace input {
    ...
    void Camera::resetSpinnakerCamera(std::map<std::string, std::unique_ptr<SpinnakerImageEvent>>::iterator& camera,
        ...
        if (!utility::vision::setEnumParam(nodeMap, "ExposureAuto", (std::isfinite(exposure) ? "Off" : "Continuous"))) {
            log("Failed to set auto exposure mode for camera",
                camera->first,
                "to",
                (std::isfinite(exposure) ? "'Off'" : "'Continuous'"));
        }
        ...
        if (std::isfinite(exposure)) {
            ...
            if (!utility::vision::setNumericParam(nodeMap, "ExposureTime", exposure)) {
                log("Failed to set exposure time for camera", camera->first, "to", exposure, "us.");
            }
        }
        ...
        if (!utility::vision::setEnumParam(nodeMap, "GainAuto", (std::isfinite(gain) ? "Off" : "Continuous"))) {
            log("Failed to set auto gain mode for camera",
                camera->first,
                "to",
                (std::isfinite(gain) ? "'Off'" : "'Continuous'"));
        }

        if (std::isfinite(gain)) {
            if (!utility::vision::setNumericParam(nodeMap, "Gain", gain)) {
                log("Failed to set gain time for camera", camera->first, "to", gain);
            }
        }
        ...
        */
        ...
        // Set black level.
        double blackLevel = config["settings"]["black_level"].as<Expression>();

        if (!utility::vision::setEnumParam(
                nodeMap,
                "BlackLevelAuto",
                ((std::isfinite(blackLevel) && (blackLevel != 0.0)) ? "Off" : "Continuous"))) {
        ...
        if (std::isfinite(blackLevel) && (blackLevel != 0.0)) {
            if (!utility::vision::setNumericParam(nodeMap, "BlackLevel", blackLevel)) {
                log("Failed to set black level for camera", camera->first, "to", blackLevel);
            }
        }
