...
namespace motion {
    ...
    BalanceKinematicResponse::BalanceKinematicResponse(std::unique_ptr<NUClear::Environment> environment)
        ...
        , footOffsetCoefficient(Eigen::Vector2d::Zero())
        , uLRFootOffset()
    ...
    void BalanceKinematicResponse::armRollCompensation(const Sensors& /*sensors*/) {
        ...
        if (armRollCompensationEnabled) {
            ...
            setLArmPosition(Eigen::Vector3d(getLArmPosition()[0],
                                            getLArmPosition()[1] + (shiftShoulder > 0 ? shiftShoulder : 0),
                                            getLArmPosition()[2]));
            setRArmPosition(Eigen::Vector3d(getRArmPosition()[0],
    ...
    void BalanceKinematicResponse::updateBody(const Sensors& sensors) {
        ...
        emit(std::make_unique<BalanceBodyUpdate>(
            getMotionPhase(), getLeftFootPosition(), getRightFootPosition(), getLArmPosition(), getRArmPosition()));
    }
    /*=======================================================================================================*/
    ...
    void BalanceKinematicResponse::updateUpperBody(const Sensors& sensors) {
        ...
        setLArmPosition(
            Eigen::Vector3d(getLArmPosition()[0], std::max(leftMinValue, getLArmPosition()[1]), getLArmPosition()[2]));
        setRArmPosition(
            Eigen::Vector3d(getRArmPosition()[0], std::min(rightMinValue, getRArmPosition()[1]), getRArmPosition()[2]));
    }
    /*=======================================================================================================*/
    //      NAME: localise
    /*=======================================================================================================*/
    void BalanceKinematicResponse::localise(Transform2D position) {
        ...
        self.locObject.position_cov = Eigen::Matrix<double, 2, 2>::Identity() * 0.1;  // made up
