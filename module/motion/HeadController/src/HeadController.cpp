...
namespace motion {
    ...
    HeadController::HeadController(std::unique_ptr<NUClear::Environment> environment)
        ...
        , updateHandle()
        , lastTime()
        , currentAngles(Eigen::Vector2d::Zero())
        , goalAngles(Eigen::Vector2d::Zero()) {
        ...
        updateHandle = on<Trigger<Sensors>, With<KinematicsModel>, Single, Priority::HIGH>().then(
            ...
            [this](const Sensors& sensors, const KinematicsModel& kinematicsModel) {
                ...
                Eigen::Vector3d goalHeadUnitVector_world =
                    sphericalToCartesian({1, currentAngles[0], currentAngles[1]});
                // Convert to robot space
                Eigen::Vector3d headUnitVector = goalRobotSpace
                                                     ? goalHeadUnitVector_world
                                                     : Transform3D(sensors.world).rotation() * goalHeadUnitVector_world;
