...
namespace motion {
    ...
    TorsoMotionPlanner::TorsoMotionPlanner(std::unique_ptr<NUClear::Environment> environment)
        ...
        , zmpParameters(Eigen::Vector4d::Zero())
        , zmpTime(0.0)
        , phase1Single(0.0)
        , phase2Single(0.0)
        , kinematicsModel()
    ...
    void TorsoMotionPlanner::updateTorsoPosition() {
        ...
        Transform3D torso;
        torso << uTorsoWorld.x(), uTorsoWorld.y(), bodyHeight, 0, bodyTilt, uTorsoWorld.angle();
        setTorsoPosition3D(torso);
    ...
    Eigen::Vector4d TorsoMotionPlanner::zmpTorsoCoefficients() {
        ...
        zmpCoefficients.head<2>() = zmpSolve(getSupportMass().x(),
                                             getTorsoSource().x(),
                                             ...
                                             stepTime,
                                             zmpTime);
        zmpCoefficients.tail<2>() = zmpSolve(getSupportMass().y(),
                                             getTorsoSource().y(),
    ...
    //      METHOD: zmpSolve
    /*=======================================================================================================*/
    Eigen::Vector2d TorsoMotionPlanner::zmpSolve(double zs,
                                                 double z1,
                                                 double z2,
                                                 double x1,
                                                 double x2,
